...
\centering
% TODO: 插入卷积运算示意图
% 建议内容：展示卷积核（如 3×3）在输入特征图上滑动的过程，标注输入、卷积核、输出
\fbox{\parbox{0.7\textwidth}{\centering\vspace{2.5cm}【待插入图片】\\卷积运算示意图\\（展示卷积核在输入特征图上滑动的过程）\vspace{2.5cm}}}
\caption{卷积运算示意图}
...
\centering
% TODO: 插入池化运算示意图
% 建议内容：对比展示最大池化与平均池化的计算过程
\fbox{\parbox{0.7\textwidth}{\centering\vspace{2.5cm}【待插入图片】\\池化运算示意图\\（最大池化与平均池化对比）\vspace{2.5cm}}}
\caption{池化运算示意图}
...
\centering
% TODO: 插入 FCN 网络结构图
% 建议内容：展示编码器-解码器结构、跳跃连接（FCN-8s/16s/32s对比）
\fbox{\parbox{0.85\textwidth}{\centering\vspace{3cm}【待插入图片】\\FCN 网络结构图\\（编码器-解码器结构与跳跃连接）\vspace{3cm}}}
\caption{FCN网络结构图}
...
\centering
% TODO: 插入 U-Net 网络结构图
% 建议内容：展示对称的 U 形结构，标注编码器、解码器、跳跃连接
\fbox{\parbox{0.85\textwidth}{\centering\vspace{3cm}【待插入图片】\\U-Net 网络结构图\\（对称 U 形结构与跳跃连接）\vspace{3cm}}}
\caption{U-Net网络结构图}
...
% 建议内容：展示 Backbone（如ResNet）、ASPP模块、Decoder结构，标注各组件
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3.5cm}【待插入图片】\\DeepLabV3+ 网络结构图\\（Backbone + ASPP + Decoder）\vspace{3.5cm}}}
...
原始 OCT 图像存在噪声干扰、对比度不足等问题，需要进行预处理以提升图像质量。本研究采用的预处理流程如图~\ref{fig:preprocess_pipeline}~所示，主要包括以下步骤：

\begin{figure}[htbp]
\centering
% TODO: 插入图像预处理流程图
% 建议内容：流程图展示 原始图像→裁剪→去噪→对比度增强→尺寸调整→归一化 的处理流程
\fbox{\parbox{0.85\textwidth}{\centering\vspace{2.5cm}【待插入图片】\\图像预处理流程图\\（原始图像→裁剪→去噪→对比度增强→尺寸调整→归一化）\vspace{2.5cm}}}
\caption{图像预处理流程图}
...
\centering
% TODO: 插入预处理效果对比图
% 建议内容：左右对比展示原始图像与预处理后图像，标注噪声抑制和对比度增强效果
\fbox{\parbox{0.85\textwidth}{\centering\vspace{3cm}【待插入图片】\\预处理效果对比\\（左：原始图像；右：预处理后图像）\vspace{3cm}}}
\caption{预处理效果对比}
...
本研究采用开源图像标注工具 LabelMe 进行语义分割标签的制作。LabelMe 是由麻省理工学院开发的多边形标注工具，支持多种标注格式输出，广泛应用于计算机视觉任务的数据准备工作。

\textbf{（1）标注工具介绍}：LabelMe 提供了交互式的多边形绘制功能，用户可以通过鼠标点击在图像上绘制任意形状的多边形，将目标区域精确标注出来。标注结果以 JSON 格式存储，包含图像路径、标注类别、多边形顶点坐标等信息。LabelMe 的标注界面如图~\ref{fig:labelme_interface}~所示。

\begin{figure}[htbp]
\centering
% TODO: 插入 LabelMe 标注界面截图
% 建议内容：展示 LabelMe 软件界面，包含标注工具栏、图像显示区域、标注的熔深区域
\fbox{\parbox{0.85\textwidth}{\centering\vspace{3cm}【待插入图片】\\LabelMe 标注界面\\（展示软件界面与熔深区域标注过程）\vspace{3cm}}}
\caption{LabelMe标注界面}
\label{fig:labelme_interface}
\end{figure}

\textbf{（2）标注类别定义}：根据焊接 OCT 图像的特点，本研究定义了以下标注类别：
\begin{itemize}
    \item \textbf{熔深线段（Weld\_Depth）}：表示焊接熔池底部的深度轮廓线，是本研究的主要分割目标，在标注时沿熔深轮廓绘制多边形区域。
    \item \textbf{背景（Background）}：除熔深线段外的所有区域，包括空气区域、材料表面以及噪声区域。
\end{itemize}

\textbf{（3）标注规范}：为保证标注的一致性和准确性，制定了以下标注规范：
\begin{enumerate}
    \item 熔深线段采用多边形方式标注，沿熔池底部轮廓紧密绘制；
    \item 对于模糊或不确定的区域，参考相邻帧图像进行判断；
    \item 标注边界应尽量精确贴合熔深轮廓，避免标注过大或过小；
    \item 所有图像由同一标注人员完成初始标注，以减少主观差异。
\end{enumerate}

\textbf{（4）标注质量控制}：为确保标注质量，采用双人交叉验证的方式进行质量控制。首先由标注人员完成初始标注，随后由第二人进行检查和修正。对于存在争议的样本，由两人协商确定最终标注结果。

标注完成后，使用 LabelMe 提供的转换脚本将 JSON 格式的标注文件转换为 PNG 格式的掩码图像，供深度学习模型训练使用。标注样例如图~\ref{fig:annotation_example}~所示。

\begin{figure}[htbp]
\centering
% TODO: 插入标注样例展示图
% 建议内容：展示原始图像与对应的标注掩码图像对比，突出熔深区域的标注结果
\fbox{\parbox{0.85\textwidth}{\centering\vspace{3cm}【待插入图片】\\标注样例展示\\（左：原始图像；右：对应标注掩码）\vspace{3cm}}}
\caption{标注样例展示}
...
\centering
% TODO: 插入数据增强效果图
% 建议内容：展示原始图像与各种增强操作后的图像对比（翻转、旋转、亮度调整等）
\fbox{\parbox{0.85\textwidth}{\centering\vspace{3cm}【待插入图片】\\数据增强效果示例\\（展示原始图像与各种增强操作后的效果对比）\vspace{3cm}}}
\caption{数据增强效果示例}
