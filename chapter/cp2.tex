...
为科学评估模型性能，本研究将数据集按照 8:1:1 的比例划分为训练集、验证集和测试集。划分时遵循以下原则：

\begin{enumerate}
    \item \textbf{时序连续性}：来自同一焊接序列的图像不跨集划分，避免信息泄露；
    \item \textbf{分布一致性}：确保三个子集中不同工况下的样本比例基本一致；
    \item \textbf{随机性}：在满足上述约束的前提下，采用随机采样方式进行划分。
\end{enumerate}
